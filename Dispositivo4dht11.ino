 ...
 * na demostracao este dispositivo estava localizado na cidade de Pesqueira-PE enquanto o dispositivo 5 estava 
 * na cidade de Russas-CE e os dispositivos 1, 2 e 3 na cidade de Mossoró-RN
 * 
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.subscribe("Temperatura_Pesqueira");
      client.subscribe("Umidade_Pesqueira");
...
void loop() {
 ...
 if (isnan(h) || isnan(t) || isnan(f)) { Serial.println (F("Falha na leitura do sendor DHT11")); return;    }
  ...
  client.publish("Temperatura_Pesqueira", temperatureTemp); 
  client.publish("Umidade_Pesqueira", humidityTemp);  }
