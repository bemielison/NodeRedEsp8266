/*
 * este sketch lê o estado do sensor de temperatura e umidade DHT-11 e publica em topicos. Ainda, possui um rele que pode ser acionado remotamente
 * a partir do dashboard node-red ou do botao fisico no dispositivo 2. 
 * 
...
void callback(String topic, byte* message, unsigned int length) {  Serial.print("Message arrived on topic: ");  
  ...
  if(topic=="Iluminacao"){
    if( (messageAtual == "Ligar") ){ digitalWrite(rele, HIGH);
       client.publish("StatusIluminacao", "Ligada");}    
    
    else if( (messageAtual == "Desligar") ){   digitalWrite(rele, LOW);
       client.publish("StatusIluminacao", "Desligada"); }}
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.subscribe("Temperatura_Mossoro");
      client.subscribe("Umidade_Mossoro");
      client.subscribe("Iluminacao");
      client.subscribe("StatusIluminacao");
...
void loop() {
 ...
 if (isnan(h) || isnan(t) || isnan(f)) { Serial.println (F("Falha na leitura do sendor DHT11")); return;    }
  ...
  client.publish("Temperatura_Mossoro", temperatureTemp); client.publish("Umidade_Mossoro", humidityTemp);  }
